...
<body data-booked-times="{{ htmlspecialchars(json_encode($bookedTimes)) }}">
    <form action="{{ route('appointments.update', ['id' => $appointment->id]) }}" method="post" id="appointmentForm">
        @csrf
        @method('put')
        ...
        <select name="day" id="day">
            @foreach ($schedules as $schedule)
                <option value="{{ $schedule->day_of_week }}" {{ $schedule->day_of_week == $appointment->day_of_week ? 'selected' : '' }} data-start="{{ $schedule->start_time }}" data-end="{{ $schedule->end_time }}">{{ $schedule->day_of_week }}</option>
            @endforeach
        </select>
